...
const PageNavigation = ({ nowPage, maxPage, baseLink }: PageNavigationProps) => {
  return (
    <section className="relative w-full flex items-center mt-6">
      {nowPage == 1 || (
      ...
      <div className="absolute left-1/2 -translate-x-1/2 justify-center items-center align-top space-x-2 text-xl sm:text-2xl">
        <span>{nowPage}</span> <span className="text-lg">of</span> <span>{maxPage}</span>
      </div>
      {nowPage == maxPage || (
        <Link href={baseLink + "page/" + (nowPage + 1)} passHref>
          <a className="inline-block ml-auto">
            <PageButton>
